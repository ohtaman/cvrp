...
def get_route(slave_model, y_opt):
    objective = pulp.lpSum(dists*x) - pulp.lpDot(y_opt, w) - y_opt[0]*n_vehicles
...
def solve(instance, timelimit, log_dir):
    ...
    slave_model = build_slave_model(n_nodes, demands, capacity, dists)
    ...
    while start + timelimit > time.time():
        ...
        route = get_route(slave_model, y_opt)
        if route is None:
            break
        routes.append(route)
        update_master_model(master_model, y, route, capacity)
